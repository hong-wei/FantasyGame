...
bool player::PickUpInventory(inventory &NewInventory,board &CurrentState){
    ...
    if(PlayerType.Strength+NewInventory.SubStrength >= 0){
        ...
        cerr<<"****note : The strength is not enough ,you can not pick up the items";
        PickFlag = 0;
    }
    return PickFlag ;
};
...
void player::DropInventory(inventory &NewInventory,const int &DropPrintNum){
        ...
        switch (DropPrintNum) {
		    ...
		    case 4://ring
                if(PlayerRing.InvenType== "LifeRing"&& LifeRingNumber >= 0){
                    ...
                    --LifeRingNumber;
                }else if(PlayerRing.InvenType == "StrengthRing" && StrengthRingNumber >= 0){
                    DropSubAttributes(PlayerRing);
                    --StrengthRingNumber;
                }else{
                    cout<<"***You have no rings!!!";
                };
		        break;
		    default:
		        break;
